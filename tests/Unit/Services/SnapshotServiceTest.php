...
describe('SnapshotService', function (): void {
    ...
    describe('Integration Scenarios', function (): void {
        test('saves and loads complete workflow', function (): void {
            ...
            expect($loaded['results']['workflow:1:0:0']['expected_valid'])->toBeTrue();
            expect($loaded['results']['workflow:1:0:0']['actual_valid'])->toBeTrue();

            expect($loaded['results']['workflow:1:0:1']['passed'])->toBeTrue();
            expect($loaded['results']['workflow:1:0:1']['expected_valid'])->toBeFalse();
            expect($loaded['results']['workflow:1:0:1']['actual_valid'])->toBeFalse();

            expect($loaded['results']['workflow:1:1:0']['passed'])->toBeFalse();
            expect($loaded['results']['workflow:1:1:0']['expected_valid'])->toBeTrue();
            expect($loaded['results']['workflow:1:1:0']['actual_valid'])->toBeFalse();
        });
