...
describe('XmlRenderer', function (): void {
    describe('render method', function (): void {
        ...
        test('includes failures when includeFailures is true and failures exist', function (): void {
            ...
            expect($failure->getElementsByTagName('id')->item(0)->textContent)->toBe('t2')
                ...
                ->and($failure->getElementsByTagName('expected_valid')->item(0)->textContent)->toBe('true')
                ->and($failure->getElementsByTagName('actual_valid')->item(0)->textContent)->toBe('false');
        ...
        test('returns early when saveXML returns false', function (): void {
            // Arrange
            $output = new BufferedOutput();
            $renderer = new XmlRenderer($output);

            // Create a suite that should produce output
            $suite = new TestSuite(
                name: 'Test Suite',
                results: [TestResult::pass('t1', 'f1', 'g1', 'd1', [], true)],
                duration: 1.0,
            ...
            // We can't actually make saveXML return false easily in normal circumstances,
            // but we can verify the code path exists by checking the behavior
            // This test documents the expected behavior when saveXML fails
            ...
            // Normal case - XML should be produced
    ...
    describe('formatSuite method', function (): void {
        ...
        test('includes failure details when includeFailures is true', function (): void {
            ...
            expect($failure1->getElementsByTagName('id')->item(0)->textContent)->toBe('test-1')
                ...
                ->and($failure1->getElementsByTagName('expected_valid')->item(0)->textContent)->toBe('true')
                ->and($failure1->getElementsByTagName('actual_valid')->item(0)->textContent)->toBe('false');
            ...
            expect($failure2->getElementsByTagName('id')->item(0)->textContent)->toBe('test-2')
                ...
                ->and($failure2->getElementsByTagName('expected_valid')->item(0)->textContent)->toBe('true')
                ->and($failure2->getElementsByTagName('actual_valid')->item(0)->textContent)->toBe('false');
    ...
    describe('formatFailure method', function (): void {
        ...
        test('formats failure with expected false and actual true', function (): void {
            ...
            $failure = $dom->getElementsByTagName('failure')->item(0);
            expect($failure->getElementsByTagName('expected_valid')->item(0)->textContent)->toBe('false')
                ->and($failure->getElementsByTagName('actual_valid')->item(0)->textContent)->toBe('true');
        });
    });
