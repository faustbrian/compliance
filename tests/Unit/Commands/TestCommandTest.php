...
PHP,
        ...
        expect($commandTester->getDisplay())->toContain('Valid formats: text, json, yaml, xml');
...
PHP,
    ...
    test('executes with YAML output format', function (): void {
        $originalCwd = getcwd();
        chdir($this->testDir);
...
PHP,
        ...
        $exitCode = $commandTester->execute(['--format' => 'yaml']);
    ...
    test('executes with YAML output format and failures', function (): void {
        $originalCwd = getcwd();
        chdir($this->testDir);
...
PHP,
        ...
        $exitCode = $commandTester->execute(['--format' => 'yaml', '--failures' => true]);
