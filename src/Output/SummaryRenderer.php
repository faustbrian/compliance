...
final readonly class SummaryRenderer
{
    ...
    private function renderSummary(array $suites): void
    {
        ...
        if ($allPassed) {
            ...
            $maxValueWidth = max(array_map(fn ($item): int => mb_strlen($item['value']), $summaryItems));
            $totalWidth = 60;

            foreach ($summaryItems as $item) {
                $paddedValue = str_pad($item['value'], $maxValueWidth, "\u{00A0}", \STR_PAD_LEFT);
                $dotsNeeded = $totalWidth - mb_strlen($item['label']) - $maxValueWidth - 1;
