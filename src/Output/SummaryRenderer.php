...
final readonly class SummaryRenderer
{
    ...
    private function renderSummary(array $suites): void
    {
        ...
        // Calculate stats width (number width + " tests")
        $statsWidth = $numberWidth + mb_strlen(' tests');

        // Render Total
        $dotsNeeded = max(1, $totalWidth - mb_strlen('Total') - $statsWidth - 1);
        $dots = str_repeat('.', $dotsNeeded);
        render(sprintf(
            <<<'HTML'
                <div class="mx-2">
        ...
        // Calculate percentage widths for consistent alignment
        $passedPercentageStr = sprintf('(%.1f%%)', $passedPercentage);
        $failedPercentageStr = sprintf('(%.1f%%)', $failedPercentage);
        $percentageWidth = max(mb_strlen($passedPercentageStr), mb_strlen($failedPercentageStr));

        // Render Passed
        $dotsNeeded = max(1, $totalWidth - mb_strlen('Passed') - $statsWidth - $percentageWidth - 1);
        $dots = str_repeat('.', $dotsNeeded);
        render(sprintf(
            <<<'HTML'
                <div class="mx-2">
        ...
        // Render Failed
        $dotsNeeded = max(1, $totalWidth - mb_strlen('Failed') - $statsWidth - $percentageWidth - 1);
        $dots = str_repeat('.', $dotsNeeded);
        render(sprintf(
            <<<'HTML'
                <div class="mx-2">
